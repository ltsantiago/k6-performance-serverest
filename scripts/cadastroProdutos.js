...
export default function () {
  //Login para obter o token de autorização
  const loginRes = http.post(`${BASE_URL}/login`, JSON.stringify(USER), {
  ...
  check(loginRes, {
    ...
    "mensagem correta": (r) =>
      r.json().message === "Login realizado com sucesso",
  ...
  const cadastroRes = http.post(
    `${BASE_URL}/produtos`,
    JSON.stringify(payloadProduto),
    { headers },
  );

  console.log(cadastroRes.json().message);

  // Verificações do cadastro de produto
  check(cadastroRes, {
